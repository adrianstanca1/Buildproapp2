        ...
        if ( file && projectId )
        {
            ...
            try
            {
                ...
                if ( import.meta.env.VITE_SUPABASE_URL )
                {
                    try
                    {
                        url = await uploadFile( file, 'documents', projectId );
                    } catch ( err )
                    {
                        console.warn( "Supabase upload failed, falling back to local reader", err );
                    }
                }

                const createDoc = ( fileUrl: string ) =>
                {
                ...
                // Fallback to local Data URL if Supabase fails or not configured
                if ( !url )
                {
                    const reader = new FileReader();
                    reader.onloadend = () =>
                    {
                        createDoc( reader.result as string );
                    };
                ...
                <div className="flex justify-between items-end mb-6">
                    ...
                    { projectId && (
                        <button
                            ...
                            className="flex items-center gap-2 bg-[#0f5c82] text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-[#0c4a6e] shadow-lg shadow-blue-900/10 transition-all transform hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
