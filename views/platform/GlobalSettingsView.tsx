                                            ...
                                            <div className="flex gap-3">
                                                <div className="flex-1 relative">
                                                    ...
                                                    <input
                                                        ...
                                                        type="password"
                                                        value={value}
                                                        className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl px-9 py-2 text-xs font-mono outline-none"
                                                    />
                                                </div>
                                                <button className="px-4 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-xs font-bold hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors">Rotate Key</button>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        {[
                                            { title: 'Global 2FA Enforcement', desc: 'Require 2FA for all SuperAdmin and Company Admin accounts.', enabled: true },
                                            { title: 'Session IP Lock', desc: 'Restricts active sessions to the IP they were created on.', enabled: false },
                                            { title: 'Strict CSP Policy', desc: 'Enable maximum protection against XSS and injection attacks.', enabled: true },
                                            { title: 'Audit Detail Persistence', desc: 'Store raw request/response payloads in logs.', enabled: false }
                                        ].map((item, idx) => (
                                            <div key={idx} className="p-4 bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-700 rounded-2xl flex items-center justify-between">
                                                <div>
                                                    <p className="text-sm font-bold text-zinc-900 dark:text-white">{item.title}</p>
                                                    <p className="text-xs text-zinc-500">{item.desc}</p>
                                                </div>
                                                <button className={`w-12 h-6 rounded-full transition-colors relative ${item.enabled ? 'bg-indigo-600' : 'bg-zinc-300 dark:bg-zinc-700'}`}>
                                                    <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${item.enabled ? 'translate-x-7' : 'translate-x-1'}`} />
                                                </button>
                                            </div>
                                        ))}
                                    </div>
