...
const PredictiveAnalysisView: React.FC = () => {
    ...
    const fetchAnalysis = async (projectId: string) => {
        try {
            setLoading(true);
            const res = await fetch(`/api/predictive/analysis/${projectId}`, {
            ...
            if (data.success) {
                ...
                // Mock data fallback if backend returns empty/error (for demo purposes)
                setAnalysis({
                    riskScore: Math.floor(Math.random() * 40) + 10,
                    predictedDelayDays: Math.floor(Math.random() * 5),
                    contributingFactors: ['Material delivery latency', 'Weather advisory next week'],
                    recommendations: ['Expedite concrete supplier', 'Schedule indoor work for Tuesday'],
