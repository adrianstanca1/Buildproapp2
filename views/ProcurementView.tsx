...
const ProcurementView: React.FC = () => {
  ...
  const handleGeneratePO = () => {
    ...
    setTimeout(() => {
      ...
      addToast("Purchase Order #PO-2025-889 created and sent for approval.", "success");
    }, 2000);
  };
      ...
      <div className="flex gap-1 border-b border-zinc-200 mb-6">
        {['VENDORS', 'ORDERS', 'APPROVALS'].map(tab => (
          <button
            ...
            className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === tab
            ...
            <tbody className="divide-y divide-zinc-100">
              {vendors.map((v, i) => (
                <tr key={i} className="hover:bg-zinc-50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${v.status === 'Preferred' ? 'bg-purple-100 text-purple-700' :
              ...
              filteredOrders.map(po => (
                <div key={po.id} onClick={() => setSelectedPO(po)} className="border border-zinc-200 rounded-lg p-4 hover:shadow-md cursor-pointer transition-all">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className={`px-3 py-1 rounded text-xs font-bold uppercase ${po.status === 'pending_approval' ? 'bg-amber-100 text-amber-700' :
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      {po.approvers.map(a => (
                        <span key={a.id} className={`text-xs px-2 py-1 rounded ${a.status === 'approved' ? 'bg-green-100 text-green-700' :
                ...
                <div key={po.id} className="border-2 border-amber-200 bg-amber-50 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    {po.approvers.map(approver => (
                      <div key={approver.id} className={`p-3 rounded border ${approver.status === 'pending' ? 'border-amber-200 bg-white' :
