...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Function to verify current session is still valid
  const verifySession = async () => {
    ...
    try {
      ...
      if (error || !session || session.access_token !== token) {
        console.warn("Session verification failed, logging out user");
        logout();
        ...
        if (timeUntilExpiry < 300) { // 5 minutes
          ...
          // Attempt to refresh the session if possible
          try {
      ...
      // Don't automatically log out on network errors, but warn
      console.warn("Could not verify session due to network error");
    }
  };
