...
export class UserManagementService {
  ...
  async inviteUser(email: string, role: UserRole, companyId: string, inviterId: string): Promise<void> {
    ...
    const userId = uuidv4();
    await db.run(
      `INSERT INTO users (id, email, name, role, status, isActive, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, email, email.split('@')[0], role, 'invited', false, now, now]
    );
    ...
    try {
      ...
      // Don't throw error as the invitation is still created
    }
    ...
    try {
      await db.run(
        `INSERT INTO audit_logs (id, userId, userName, action, resource, resourceId, changes, status, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [logId, userId, 'system', action, resource, resourceId, JSON.stringify(changes), 'success', now]
      );
    } catch (error) {
