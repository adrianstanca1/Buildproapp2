...
const pgUrl = "postgresql://postgres:Cumparavinde1@db.zpbuvuxpfemldsknerew.supabase.co:5432/postgres";

async function harmonize() {
    console.log('🐘 Connecting to PostgreSQL (zpbuvuxpfemldsknerew) for schema harmonization...');
    const pool = new Pool({
    ...
    try {
        for (const r of renames) {
            ...
            `, [r.table, r.old.toLowerCase()]); // PG uses lowercase for table/column names usually
            ...
            if (checkOld.rows.length > 0) {
                ...
                if (checkNew.rows.length === 0) {
                    ...
                    await client.query(`ALTER TABLE ${r.table} RENAME COLUMN ${r.old} TO "${r.new}"`); // Use quotes for camelCase
                    console.log(`✅ Success.`);
                } else {
